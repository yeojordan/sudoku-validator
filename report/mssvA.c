...
{
    // Memory mapping
    *buff2Ptr = (int*) mmap(NULL, sizeof(int)*NINE*NINE,
                PROT_READ | PROT_WRITE, MAP_SHARED, *buff2FD, 0);
    *buff1Ptr = mmap(NULL, sizeof(int)*NUMPROCESSES,
                PROT_READ | PROT_WRITE, MAP_SHARED, *buff1FD, 0);
    *countPtr = (int*) mmap(NULL, sizeof(int),
                PROT_READ | PROT_WRITE, MAP_SHARED, *counterFD, 0);
    *semaphores = mmap(NULL, sizeof(sem_t) * 2,
                PROT_READ | PROT_WRITE, MAP_SHARED, *semFD, 0);
    *region = mmap(NULL, sizeof(Region)*NUMPROCESSES,
                PROT_READ | PROT_WRITE, MAP_SHARED, *regionFD, 0);
    *resourceCount = mmap(NULL, sizeof(int),
                PROT_READ | PROT_WRITE, MAP_SHARED, *resFD, 0);
}
